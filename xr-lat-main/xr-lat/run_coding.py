...
class ModelArguments:
    ...
    per_label_threshold: bool = field(
        default=False,
        metadata={"help": "tune per-label thresholds on validation set and use them for eval/predict"})
...
def main():
    ...
    # Enable TF32 on Ampere/Ada GPUs to speed up training (safe for FP32/FP16)
    try:
        import torch
    ...
    for i in range(num_level):
        ...
        # If only evaluating/predicting, try to load weights from last checkpoint/state dict
        if not training_args.do_train:
            ckpt_dir = get_last_checkpoint(training_args.output_dir)
            if ckpt_dir is None:
                ckpt_dir = training_args.output_dir
            loaded = False
            # Try PyTorch bin first
            try:
                state_path = os.path.join(ckpt_dir, 'pytorch_model.bin')
                if os.path.exists(state_path):
                    state_dict = torch.load(state_path, map_location='cpu')
                    hilat_model.load_state_dict(state_dict, strict=False)
                    logger.info(f"Loaded model weights from {state_path}")
                    loaded = True
            except Exception as e:
                logger.warning(f"Failed to load pytorch bin from {ckpt_dir}: {e}")
            # Try safetensors
            if not loaded:
                try:
                    ...
                    if os.path.exists(safe_path):
                        ...
                        hilat_model.load_state_dict(state_dict, strict=False)
                        logger.info(f"Loaded model weights from {safe_path}")
                        loaded = True
                except Exception as e:
                    logger.warning(f"Failed to load safetensors from {ckpt_dir}: {e}")
            if not loaded:
                logger.warning(f"No checkpoint weights loaded from {ckpt_dir}; using base pretrained weights.")
        ...
        # Handle logging when do_train=False (train_dataset may be undefined)
        try:
            active_labels = train_dataset.label_width
        except Exception:
            try:
                active_labels = eval_dataset.label_width
            except Exception:
                active_labels = model_args.num_labels if model_args.num_labels is not None else -1
        logger.info("Level: {}, active labels: {}".format(str(i), active_labels))
        ...
        # Initialize our Trainer (wrap compute_metrics to use configurable threshold)
        def compute_metrics_with_threshold(p):
            return compute_metrics(p, threshold=model_args.threshold)
        ...
        # Threshold tuning on validation set before evaluation
        thr_vec = None
        if training_args.do_eval:
            try:
                logger.info("Tuning decision threshold on validation set (optimize micro_f1)...")
                tune_out = trainer.predict(eval_dataset, metric_key_prefix="tune")
                eval_logits = tune_out.predictions[0] if isinstance(tune_out.predictions, tuple) else tune_out.predictions
                eval_labels = eval_dataset.labels
                if model_args.per_label_threshold:
                    probs = 1.0 / (1.0 + np.exp(-eval_logits))
                    num_labels = probs.shape[1]
                    thr_grid = np.linspace(0.05, 0.9, 18)
                    thr_vec = np.full((num_labels,), 0.5, dtype=float)
                    for j in range(num_labels):
                        ...
                        best_t = 0.5
                        for t in thr_grid:
                            pred = (probs[:, j] >= t).astype(float)
                            tp = (y * pred).sum()
                            fp = pred.sum() - tp
                            fn = y.sum() - tp
                            f1 = 0.0 if tp == 0 else 2.0 * tp / (2.0 * tp + fp + fn)
                            if f1 > best_f1:
                                best_f1 = f1
                                best_t = float(t)
                        thr_vec[j] = best_t
                    np.save(os.path.join(training_args.output_dir, 'per_label_thresholds.npy'), thr_vec)
                    logger.info("Per-label thresholds tuned and saved to per_label_thresholds.npy")
                else:
                    best_thr = model_args.threshold if model_args.threshold is not None else 0.5
                    best_f1 = -1.0
                    for thr in np.linspace(0.05, 0.9, 18):
                        m = compute_metrics((eval_labels, eval_logits), threshold=thr)
                        if m.get("micro_f1", -1.0) > best_f1:
                            best_f1 = m["micro_f1"]
                            best_thr = float(thr)
                    model_args.threshold = best_thr
                    logger.info(f"Best threshold on validation (micro_f1): {best_thr:.4f} (f1={best_f1:.4f})")
            except Exception as e:
                logger.warning(f"Threshold tuning failed with error: {e}. Using threshold={model_args.threshold}")

        # Evaluation
        if training_args.do_eval:
            logger.info("*** Evaluate ***")

            if model_args.per_label_threshold and thr_vec is not None:
                # Use per-label thresholds and manual evaluate
                eval_out = trainer.predict(eval_dataset, metric_key_prefix="eval")
                eval_logits = eval_out.predictions[0] if isinstance(eval_out.predictions, tuple) else eval_out.predictions
                eval_labels = eval_dataset.labels
                metrics = compute_metrics((eval_labels, eval_logits), threshold=thr_vec)
            else:
            ...
            if model_args.per_label_threshold and thr_vec is not None:
                metrics["best_threshold"] = "per_label"
            else:
                metrics["best_threshold"] = model_args.threshold
        ...
        if training_args.do_predict:
            ...
            if model_args.per_label_threshold and thr_vec is not None:
                final_test_logits = output.predictions[0] if isinstance(output.predictions, tuple) else output.predictions
                metric_scores = compute_metrics((predict_dataset.labels, final_test_logits), threshold=thr_vec)
            elif model_args.beam_search:
